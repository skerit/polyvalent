...
public class PolyvalentItemPolyGenerator {
    ...
    public static ItemPoly generatePoly(Item item, PolyRegistry builder) {
        ...
        if (item instanceof BlockItem) {
            try {
                return new CustomModelDataPoly(builder.getCMDManager(), item, Polyvalent.WOOD_BLOCK_ITEM);
            } catch (Exception e) {
        ...
        try {
            return new CustomModelDataPoly(builder.getCMDManager(), item, Polyvalent.DUMMY_ONE);
        } catch (Exception e) {
            // Ignore
        }
